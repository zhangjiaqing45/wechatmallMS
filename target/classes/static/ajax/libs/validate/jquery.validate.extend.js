...
$(document).ready(function () {
    ...
    jQuery.validator.addMethod("isPrice", function (value, element) {
        ...
        var target = /^(0|([1-9][0-9]{0,8})|(([0]\.\d{1,2}|[1-9][0-9]{0,8}\.\d{1,2})))$/;
        return this.optional(element) || (target.test(value));
    }, "请填写正确的价格（整数9位、小数2位有效数字）");
    // 密码
    jQuery.validator.addMethod("isPwd", function (value, element) {
        // var length = value.length;
        var target = /^(?=.*\d)(?=.*[a-zA-Z])(?=.*[~!@#$%^&*])[\da-zA-Z~!@#$%^&*]{10,20}$/;
        return this.optional(element) || (target.test(value));
    }, "请填写正确的密码格式（包含字母、数字、特殊字符10到20位）");
    // 折扣
    jQuery.validator.addMethod("isDiscount", function (value, element) {
        // var length = value.length;
        var target = /^((0\.[1-9]{1})|(([1-9]{1})(\.\d{1})?))$/;
        return this.optional(element) || (target.test(value));
    }, "请填写正确的折扣（大于0小于10，且允许保留一位小数点）");
    // 正整数
    jQuery.validator.addMethod("dayuLing", function (value, element) {
        // var length = value.length;
        var target = /[1-9]\d*(\.\d*[1-9])?/;
        return this.optional(element) || (target.test(value));
    }, "请填写值大于0");
    // 正整数
    ...
    jQuery.validator.addMethod("integerOrDecimal", function (value, element) {
        // var length = value.length;
        var target = /^(0|([1-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))$/;
        return this.optional(element) || (target.test(value));
    }, "请填写正确的数字格式（整数或小数）");
