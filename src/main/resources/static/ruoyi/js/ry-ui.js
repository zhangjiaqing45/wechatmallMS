...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            // 回显数据字典
            selectDictLabel: function (datas, value, display) {
                var actions = [], display = display ? display : "rich";
                $.each(datas, function (index, dict) {
                    if (dict.dictValue == ('' + value)) {
                        if ($.common.equals("rich", display)) {
                            var listClass = $.common.equals("default", dict.listClass) || $.common.isEmpty(dict.listClass) ? "" : "badge badge-" + dict.listClass;
                            actions.push($.common.sprintf("<span class='%s'>%s</span>", listClass, dict.dictLabel));
                        } else if ($.common.equals("plain", display)) {
        ...
        modal: {
            ...
            openOptions: function (options) {
                ...
                var _width = $.common.isEmpty(options.width) ? ($(window).width() - 80) : options.width;
            ...
            preview: function(src, target) {
                ...
                if ($.common.equals("self", target)) {
                    layer.open({
                        ...
                        shadeClose: true,
                        area: ['auto', 'auto'],
                        content: "<img src='" + src + "' height='auto' width='auto' style='max-width: " + maxWidth + "; max-height: " + maxHeight + "; ' />"
                    });
                } else if ($.common.equals("blank", target)) {
                    window.open(src);
                }
        ...
        operate: {
            ...
            // 富文本summernote上传图片
            summernoteImageUpload: function(file, selector, url) {
                var data = new FormData();
                url = $.common.isEmpty(url) ? ftConsts.upload.url.rich : url;
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        if (result.code == web_status.SUCCESS) {
                            $(selector).summernote('editor.insertImage', "/"+result.fileName);
                        } else {
                            $.modal.alertError(result.msg);
                        }
                    },
            ...
            // layui上传
            layuiUpload: function(elem, url, size, accept, callback) {
...
var ftConsts = {
    upload: {
        url: {
            ...
            pc: "common/upload",
            // 组织信息
            org: "common/orgupload",
            // 富文本
            rich: "common/richTextUpload",
        },
        accept: {
            // 图片
            images: "images",
    ...
    regExp: {
        // 排序：5位以内正整数
        sort: /^(0|[1-9]\d{0,4})$/,
        price : /^(0|([1-9][0-9]{0,8})|(([0]\.\d{1,2}|[1-9][0-9]{0,8}\.\d{1,2})))$/
    }
};